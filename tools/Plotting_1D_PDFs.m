...
multi = 1; % multi = 1 non constrained inversion where num_layers = 1, 
           % multi = 2 constrained MuLTI inversion,  
...
if multi == 1 %multi non constrained 
    subplot(2,3,[2 5])
...
elseif multi == 2 %multi constrained 
   % calculate fixed layer constraints for Vp and density 
    for i = 1: (num_layers-1)
        [val(i), idx(i)] = min(abs(x - priors.layer_depths(i)));
    end
    vp = zeros(dis,1);
    density = zeros(dis,1);
    for i = 1:num_layers
        if i == 1
            vp(1:idx(1),1) = priors.vp(1);
            density(1:idx(1),1) = priors.density(1);
        elseif i > 1 && i < num_layers
            vp(idx(i-1)+1:idx(i),1) = priors.vp(i);
            density(idx(i-1)+1:idx(i),1) = priors.density(i);
        else %i = num_layers
            vp(idx(i-1)+1:dis,1) = priors.vp(i);
            density(idx(i-1)+1:dis,1) = priors.density(i);
        end
    end

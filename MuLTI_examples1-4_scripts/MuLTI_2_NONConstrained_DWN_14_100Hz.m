...
load('1.2.Synthetic_DWN_picks_long_offset.mat'); %load picked dispersion curves 
freq = data(6:39,1);% set frequency
data = data(6:39,2); % set observed data
nd =numel(data); % nd is the number of data points

%determine fitting error of picking dispersion curve in m/s
fitting_error = half_width(6:39,2); % set fitting error to half width of waveform's dispersion image
...
if running_mode == 1
    ...
    %misfit_m4 = abs(data(i) - forward_model(i,6));
        if i == 14 %set first freq picked to the fundamental mode
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Set up true model to compare with final results
load('true_model_vs.mat')
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
hold on
plot(true_model_vs,x,'b','LineWidth',2);
hold on
...
hold on
plot(true_model_vs,x,'black','LineWidth',2);
%hold on
%plot(AV(:,1),x,'r','LineWidth',2);
...
% Create xlabel
xlabel('Vs (m/s)');
%h=legend('95% CI', 'true model', 'average solution');
%set(gca,'Ydir','normal')
set(h,'Interpreter','none','Location','NorthOutside');
...
save('2.NONConstrained_DWN_14_100Hz.mat') 

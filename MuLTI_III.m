...
load('vpdata.mat'); %load vp data
load('dendata.mat'); %load vp data
...
elseif num_layers == 2
    ...
    priors.PRmin(1:num_layers) = 0.1;% set Poisson's ratio boundries
...
ybulk = linspace(log10((min(priors.vpmin)^2)*priors.denmin-((4/3)*((min(priors.vsmin)^2)*priors.denmin))),...
    log10((max(priors.vpmax)^2)*priors.denmax-((4/3)*((min(priors.vsmin)^2)*priors.denmin))), dis);
yVpVs = linspace(0, 5, dis);
yPR = linspace(0, 0.5, dis);
y_misfit = linspace(1, 300, dis);
freq_x(1:length(freq),1)= freq; % freq grid for forward model PDF
freq_x(length(freq)+1,1)= freq(end,1)-freq(end-1,1)+freq(end,1); % extra point needed at end of freq grid for accurate binning
...
CI_density_M1 = zeros((dis-1),length(freq_x)-1);
CI_density_M2 = zeros((dis-1),length(freq_x)-1);
CI_density_misfit = zeros((dis-1),length(freq_x)-1);
...
% find which layer the nuclei will move to:
    ...
    if out==1
        ...
        if priors_OK == 0 
            ...
            if running_mode == 1
                ...
                min_misfit_all_modes_p = min_misfit_all_modes;
                forward_model = [freq zeros(length(data),5)];
                    try
                    forward_model = gpdc(thickness, vp.', vs.', (density*1000).', 'fV', freq.');
                    forward_model = [forward_model(:, 1), rdivide(1, forward_model(:, 2:end))]; %convert forward model into velocity (m/s)
 ...
 % (for the result is always 0).
    ...
    bulk = log10((nuclei_density.*(nuclei_vp).^2) - ((4/3).*(nuclei_density.*(nuclei_vs).^2)));
    ...
    PR = 0.5*(((VpVs.^2)-2)./((VpVs.^2)-1));
        ...
        [misfit]=histcounts2(min_misfit_all_modes_p',freq',y_misfit,freq_x');
    ...
    if (s>burn_in)
        if (mod(s,thin)==0)
            ...
            CI_density_FM = CI_density_FM + FM;
            CI_density_M1 = CI_density_M1 + M1;
            CI_density_M2 = CI_density_M2 + M2;
            CI_density_misfit = CI_density_misfit + misfit;
...
hold on
plot(Vs_true,x,'r','LineWidth',2); %true solution
...
% % hold on
% % plot(shear_true,vpdata(:,1),'r','LineWidth',2); %true solution
% title('Shear PDF');
% % Create ylabel
% ylabel('Depth (m)');
% % Create xlabel
...
% % hold on
% % plot(bulk_true,vpdata(:,1),'r','LineWidth',2); %true solution
% title('bulk PDF');
% % Create ylabel
% ylabel('Depth (m)');
% % Create xlabel
...
% % hold on
% % plot(VpVs_true,vpdata(:,1),'r','LineWidth',2); %true solution
% title('VpVs PDF');
% % Create ylabel
% ylabel('Depth (m)');
% % Create xlabel
...
% % hold on
% % plot(PR_true,vpdata(:,1),'r','LineWidth',2); %true solution
% title('PR PDF');
% % Create ylabel
% ylabel('Depth (m)');
% % Create xlabel
...
if running_mode == 1
%Superimposing all modes onto one PDF plot
sizeCI = size(CI_density_FM);
len = sizeCI(1,1);
wid = sizeCI(1,2);
CI_density_ALL = zeros(len,wid);
for i = 1:wid
    for j = 1:len
        CI_density_ALL(j,i) = nansum([CI_density_FM(j,i) CI_density_M1(j,i) CI_density_M2(j,i)]);
    end
end
%PDF ALL modes
figure
...
figure
imagesc(freq_edge,misfit_edge,CI_density_misfit) %pdf plot
hold on;
plot( freq, data.*0.05, 'k');
% Create ylabel
ylabel('Misfit(m/s)');
% Create xlabel
xlabel('Frequency (Hz)');

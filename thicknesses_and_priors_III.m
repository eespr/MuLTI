...
if num_floating_nuclei > priors.npt_max  | num_floating_nuclei < priors.npt_min
    priors_OK = 0; thickness = 0; layer_density = 0; layer_vp = 0; layer_vs = 0; %early return
    return;
end 
...
layer_vs = [nuclei_vs(I) ];
layer_vp = [nuclei_vp(I) ];
layer_density = [nuclei_density(I) ];
PR = 0.5*((((layer_vp./layer_vs).^2)-2)./(((layer_vp./layer_vs).^2)-1));
...
% Test the prior
    if layer_vs(i) < priors.vsmin(layer) || layer_vs(i) > priors.vsmax(layer) %Vs bounds
       priors_OK = 0;
       return; % early return
    end
    if PR(i) < priors.PRmin(layer) || PR(i) > priors.PRmax(layer) %PR bounds
       priors_OK = 0;
       return; % early return
    end
end

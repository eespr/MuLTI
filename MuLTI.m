...
if num_layers == 3
    priors.layer_depths(1) = 2 ; % layer 1 depth.
    priors.layer_depths(2) = 25.5 ; % layer 2 depth.
    priors.vsmin(1) = 500; % layer 1
    priors.vsmax(1) = 1700; 
    priors.vsmin(2) = 1700; % layer 2
    priors.vsmax(2) = 1950;
    priors.vsmin(3) = 200; % layer 3
    priors.vsmax(3) = 2800;
    priors.vp(1) = 2500; % layer 1 fixed vp
    priors.vp(2) = 3810; % layer 2 fixed Vp
    priors.vp(3) = 4000; % layer 3 fixed Vp
    priors.density(1) = 0.47; % layer 1 fixed density
    priors.density(2) = 0.92; % layer 2 fixed density
    priors.density(3) = 2.5; % layer 3 fixed density
elseif num_layers == 2
    priors.layer_depths(1) = 2 ; % layer 1 depth.
    priors.vsmin(1) = 500; % layer 1
    priors.vsmax(1) = 1700;
    priors.vsmin(2) = 200; % layer 3
    priors.vsmax(2) = 2800;
    priors.vp(1) = 2500; % layer 1 fixed vp    
    priors.vp(2) = 4000; % layer 3 fixed Vp
    priors.density(1) = 0.47; % layer 1 fixed density    
    priors.density(2) = 2.5; % layer 3 fixed density
else % num_layers = 1 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Set up true model to compare with final results (only for synthetic tests!)
%load('enter_true_vs_model.mat')
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
hold on
plot(best_model_vs,x,'k','LineWidth',2); %best solution
hold on
plot(inbetween, x2,'g'); %95% credible interval
hold on
plot(vs_mode_n0,x,'k','LineWidth',2); % mode solution
%hold on
%plot(true_model_vs,x,'black','LineWidth',2); %only for synthetic tests
hold on
